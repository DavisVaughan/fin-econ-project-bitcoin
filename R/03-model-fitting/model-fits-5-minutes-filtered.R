...
# Apply a +/- 5 std deviation filter, replace with the median
returns <- returns %>%
  mutate(return = replace(return, 
                          !(return >= median(return, na.rm = T) - sd(return, na.rm = T) * 5 & return <= median(return, na.rm = T) + sd(return, na.rm = T) * 5), 
                          median(return, na.rm = T)))
...
saveRDS(forecast, "models/garch-model-5-min-filtered.rds")

# Save all forecast information
saveRDS(daily_forecast, "forecasts/forecast-5-min-filtered.rds")
